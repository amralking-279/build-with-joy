        ...
        <div className="flex gap-2">
          {(['all', 'meccan', 'medinan'] as FilterType[]).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`surah-filter-btn px-4 py-2 rounded-xl text-sm font-cairo transition-all duration-200 ${
                filter === f
                  ? 'surah-filter-btn--active bg-emerald-600 text-white border border-emerald-500 font-bold shadow-sm'
