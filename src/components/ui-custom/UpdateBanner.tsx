...
const UPDATE_VERSION = '2026-06-25.prophets-stories';
const STORAGE_KEY = 'quran_update_banner_seen';
const AUTO_DISMISS_MS = 12000;
const SWIPE_THRESHOLD = 80;

const UPDATES: string[] = [
  'إضافة قسم «قصص الأنبياء» 📖 — قصص الأنبياء الخمسة والعشرين مع الآيات والعبر والاستماع بصوت راوٍ',
  'أيقونة «الشكل» 🎨 أسفل الشاشة لتغيير شكل التطبيق: اللون، نمط العرض، وقالب الفجر',
  'تفعيل زر المشاركة في قالب الفجر ليفتح قائمة المشاركة وتختار التطبيق',
  'إصلاح الأزرار والنصوص المختفية في الوضع الليلي لقالب الفجر ومواقيت الصلاة',
  'أزرار التصفية (الكل / مكية / مدنية) في صفحة القرآن بخلفية بيضاء ونص أسود',
  'تصحيح ذكر «سبحان الله العظيم» في السبحة بإزالة «وبحمده»',
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          dir="rtl"
        >
          <div className="max-w-3xl mx-auto rounded-2xl border border-emerald-800 bg-[#06140a] overflow-hidden shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)]" style={safeLayerStyle}>
            <div className="relative p-4 sm:p-5">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl bg-emerald-900 border border-emerald-700 flex items-center justify-center shrink-0" style={safeLayerStyle}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-3 mb-2">
                    <div>
                      ...
                      <p className="text-emerald-400 text-xs font-cairo">آخر ما أُضيف للتطبيق</p>
                    </div>
                    <button
                      onClick={close}
                      className="w-8 h-8 rounded-lg bg-emerald-950 border border-emerald-800 hover:brightness-125 text-emerald-200 flex items-center justify-center transition-colors shrink-0"
                      style={safeLayerStyle}
                      aria-label="إغلاق"
                    >
                      <X className="w-4 h-4" />
                  ...
                  <ul className="space-y-1.5 mt-3">
                    {UPDATES.map((u, i) => (
                      <motion.li
                        ...
                        className="flex items-start gap-2 text-emerald-50 font-cairo text-xs sm:text-sm"
                      >
                  ...
                  <button
                    onClick={close}
                    className="mt-4 w-full sm:w-auto bg-emerald-700 border border-emerald-600 hover:bg-emerald-600 text-white font-cairo text-sm rounded-lg px-5 py-2 transition-colors"
                    style={safeLayerStyle}
                  >
                    تم، شكراً
                  </button>
