...
export const reviewPronunciation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    // Same-origin check (cheap first line of defence — easily spoofed by
    // non-browser clients, so it is NOT the primary control).
    ...
    // Real anti-abuse layer: per-IP server-side quota. Caps cost exposure of
    // the shared LOVABLE_API_KEY for scripted/non-browser callers that can
    // forge Origin/Referer headers.
    const { rateLimit } = await import("@/lib/server/rate-limit.server");
    const ip =
      getRequestIP({ xForwardedFor: true }) ||
      getRequestHeader("cf-connecting-ip") ||
      "unknown";
    const limited = rateLimit("pronunciation", ip, 20, 60_000);
    if (!limited.ok) {
      throw new Error("لقد تجاوزت الحد المسموح، حاول بعد قليل");
    }
