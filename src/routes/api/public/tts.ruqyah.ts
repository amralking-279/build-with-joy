...
export const Route = createFileRoute('/api/public/tts/ruqyah')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        // Same-origin first line of defence (browsers can't forge Origin
        // cross-site; non-browser clients can — rate limiting below is the
        // real control).
        ...
        // Per-IP server-side quota caps cost exposure of the shared
        // LOVABLE_API_KEY against scripted abuse.
        const { rateLimit, getClientIp } = await import(
          '@/lib/server/rate-limit.server'
        );
        const ip = getClientIp(request.headers);
        const limited = rateLimit('tts-ruqyah', ip, 10, 60_000);
        if (!limited.ok) {
